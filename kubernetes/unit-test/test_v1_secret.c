...
void test_v1_secret(int include_optional) {
	...
	mazu_cJSON* jsonv1_secret_1 = v1_secret_convertToJSON(v1_secret_1);
	printf("v1_secret :\n%s\n", mazu_cJSON_Print(jsonv1_secret_1));
	v1_secret_t* v1_secret_2 = v1_secret_parseFromJSON(jsonv1_secret_1);
	mazu_cJSON* jsonv1_secret_2 = v1_secret_convertToJSON(v1_secret_2);
	printf("repeating v1_secret:\n%s\n", mazu_cJSON_Print(jsonv1_secret_2));
}
