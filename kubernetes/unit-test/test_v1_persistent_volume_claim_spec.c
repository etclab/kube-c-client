...
#include "test_v1_typed_object_reference.c"
#include "test_v1_volume_resource_requirements.c"
  ...
  if (include_optional) {
    v1_persistent_volume_claim_spec = v1_persistent_volume_claim_spec_create(
      ...
      instantiate_v1_typed_local_object_reference(0),
       // false, not to have infinite recursion
      instantiate_v1_typed_object_reference(0),
       // false, not to have infinite recursion
      instantiate_v1_volume_resource_requirements(0),
       // false, not to have infinite recursion
...
void test_v1_persistent_volume_claim_spec(int include_optional) {
	...
	mazu_cJSON* jsonv1_persistent_volume_claim_spec_1 = v1_persistent_volume_claim_spec_convertToJSON(v1_persistent_volume_claim_spec_1);
	printf("v1_persistent_volume_claim_spec :\n%s\n", mazu_cJSON_Print(jsonv1_persistent_volume_claim_spec_1));
	...
	mazu_cJSON* jsonv1_persistent_volume_claim_spec_2 = v1_persistent_volume_claim_spec_convertToJSON(v1_persistent_volume_claim_spec_2);
	printf("repeating v1_persistent_volume_claim_spec:\n%s\n", mazu_cJSON_Print(jsonv1_persistent_volume_claim_spec_2));
}
