...
void test_v1_pod_ip(int include_optional) {
	...
	mazu_cJSON* jsonv1_pod_ip_1 = v1_pod_ip_convertToJSON(v1_pod_ip_1);
	printf("v1_pod_ip :\n%s\n", mazu_cJSON_Print(jsonv1_pod_ip_1));
	v1_pod_ip_t* v1_pod_ip_2 = v1_pod_ip_parseFromJSON(jsonv1_pod_ip_1);
	mazu_cJSON* jsonv1_pod_ip_2 = v1_pod_ip_convertToJSON(v1_pod_ip_2);
	printf("repeating v1_pod_ip:\n%s\n", mazu_cJSON_Print(jsonv1_pod_ip_2));
}
