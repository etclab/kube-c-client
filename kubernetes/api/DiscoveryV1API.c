...
DiscoveryV1API_createNamespacedEndpointSlice(apiClient_t *apiClient, char *_namespace, v1_endpoint_slice_t *body, char *pretty, char *dryRun, char *fieldManager, char *fieldValidation)
{
    ...
    // create the path
    long sizeOfPath = strlen("/apis/discovery.k8s.io/v1/namespaces/{namespace}/endpointslices")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/apis/discovery.k8s.io/v1/namespaces/{namespace}/endpointslices");
...
DiscoveryV1API_deleteNamespacedEndpointSlice(apiClient_t *apiClient, char *name, char *_namespace, char *pretty, char *dryRun, int *gracePeriodSeconds, int *orphanDependents, char *propagationPolicy, v1_delete_options_t *body)
{
    ...
    // create the path
    long sizeOfPath = strlen("/apis/discovery.k8s.io/v1/namespaces/{namespace}/endpointslices/{name}")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/apis/discovery.k8s.io/v1/namespaces/{namespace}/endpointslices/{name}");
...
DiscoveryV1API_getAPIResources_17(apiClient_t *apiClient)
{
    ...
    // create the path
    long sizeOfPath = strlen("/apis/discovery.k8s.io/v1/")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/apis/discovery.k8s.io/v1/");
...
DiscoveryV1API_listEndpointSliceForAllNamespaces(apiClient_t *apiClient, int *allowWatchBookmarks, char *_continue, char *fieldSelector, char *labelSelector, int *limit, char *pretty, char *resourceVersion, char *resourceVersionMatch, int *sendInitialEvents, int *timeoutSeconds, int *watch)
{
    ...
    // create the path
    long sizeOfPath = strlen("/apis/discovery.k8s.io/v1/endpointslices")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/apis/discovery.k8s.io/v1/endpointslices");
