...
CoordinationV1API_createNamespacedLease(apiClient_t *apiClient, char *_namespace, v1_lease_t *body, char *pretty, char *dryRun, char *fieldManager, char *fieldValidation)
{
    ...
    // create the path
    long sizeOfPath = strlen("/apis/coordination.k8s.io/v1/namespaces/{namespace}/leases")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/apis/coordination.k8s.io/v1/namespaces/{namespace}/leases");
    ...
    {
        //string
        localVarSingleItemJSON_body = v1_lease_convertToJSON(body);
    ...
    mazu_cJSON *CoordinationV1APIlocalVarJSON = mazu_cJSON_Parse(apiClient->dataReceived);
    v1_lease_t *elementToReturn = v1_lease_parseFromJSON(CoordinationV1APIlocalVarJSON);
    mazu_cJSON_Delete(CoordinationV1APIlocalVarJSON);
...
CoordinationV1API_deleteNamespacedLease(apiClient_t *apiClient, char *name, char *_namespace, char *pretty, char *dryRun, int *gracePeriodSeconds, int *orphanDependents, char *propagationPolicy, v1_delete_options_t *body)
{
    ...
    // create the path
    long sizeOfPath = strlen("/apis/coordination.k8s.io/v1/namespaces/{namespace}/leases/{name}")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/apis/coordination.k8s.io/v1/namespaces/{namespace}/leases/{name}");
...
CoordinationV1API_getAPIResources_15(apiClient_t *apiClient)
{
    ...
    // create the path
    long sizeOfPath = strlen("/apis/coordination.k8s.io/v1/")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/apis/coordination.k8s.io/v1/");
...
CoordinationV1API_listLeaseForAllNamespaces(apiClient_t *apiClient, int *allowWatchBookmarks, char *_continue, char *fieldSelector, char *labelSelector, int *limit, char *pretty, char *resourceVersion, char *resourceVersionMatch, int *sendInitialEvents, int *timeoutSeconds, int *watch)
{
    ...
    // create the path
    long sizeOfPath = strlen("/apis/coordination.k8s.io/v1/leases")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/apis/coordination.k8s.io/v1/leases");
