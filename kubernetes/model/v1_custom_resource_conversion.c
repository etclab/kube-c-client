...
mazu_cJSON *v1_custom_resource_conversion_convertToJSON(v1_custom_resource_conversion_t *v1_custom_resource_conversion) {
    ...
    if(mazu_cJSON_AddStringToObject(item, "strategy", v1_custom_resource_conversion->strategy) == NULL) {
    goto fail; //String
    }
    ...
    if(v1_custom_resource_conversion->webhook) {
    mazu_cJSON *webhook_local_JSON = v1_webhook_conversion_convertToJSON(v1_custom_resource_conversion->webhook);
    if(webhook_local_JSON == NULL) {
    goto fail; //model
    }
    mazu_cJSON_AddItemToObject(item, "webhook", webhook_local_JSON);
    if(item->child == NULL) {
    goto fail;
    }
    }
...
v1_custom_resource_conversion_t *v1_custom_resource_conversion_parseFromJSON(mazu_cJSON *v1_custom_resource_conversionJSON){
    ...
    // v1_custom_resource_conversion->strategy
    mazu_cJSON *strategy = mazu_cJSON_GetObjectItemCaseSensitive(v1_custom_resource_conversionJSON, "strategy");
    if (!strategy) {
        goto end;
    }

    
    if(!mazu_cJSON_IsString(strategy))
    {
    goto end; //String
    }

    // v1_custom_resource_conversion->webhook
    mazu_cJSON *webhook = mazu_cJSON_GetObjectItemCaseSensitive(v1_custom_resource_conversionJSON, "webhook");
    if (webhook) { 
