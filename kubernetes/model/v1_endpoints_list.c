...
v1_endpoints_list_t *v1_endpoints_list_create(
    ...
    v1_endpoints_list_local_var->api_version = api_version;
...
void v1_endpoints_list_free(v1_endpoints_list_t *v1_endpoints_list) {
    if(NULL == v1_endpoints_list){
        return ;
    }
    listEntry_t *listEntry;
    if (v1_endpoints_list->api_version) {
        free(v1_endpoints_list->api_version);
        v1_endpoints_list->api_version = NULL;
    }
    if (v1_endpoints_list->items) {
        list_ForEach(listEntry, v1_endpoints_list->items) {
            v1_endpoints_free(listEntry->data);
        }
        list_freeList(v1_endpoints_list->items);
        v1_endpoints_list->items = NULL;
    }
    if (v1_endpoints_list->kind) {
        free(v1_endpoints_list->kind);
        v1_endpoints_list->kind = NULL;
    }
    if (v1_endpoints_list->metadata) {
        v1_list_meta_free(v1_endpoints_list->metadata);
        v1_endpoints_list->metadata = NULL;
    }
    free(v1_endpoints_list);
}

mazu_cJSON *v1_endpoints_list_convertToJSON(v1_endpoints_list_t *v1_endpoints_list) {
    mazu_cJSON *item = mazu_cJSON_CreateObject();

    // v1_endpoints_list->api_version
    if(v1_endpoints_list->api_version) {
    if(mazu_cJSON_AddStringToObject(item, "apiVersion", v1_endpoints_list->api_version) == NULL) {
    goto fail; //String
    }
    }
    ...
    mazu_cJSON *itemLocal = v1_endpoints_convertToJSON(itemsListEntry->data);
    if(itemLocal == NULL) {
    goto fail;
    }
    ...
    if(v1_endpoints_list->kind) {
    if(mazu_cJSON_AddStringToObject(item, "kind", v1_endpoints_list->kind) == NULL) {
    goto fail; //String
    }
    }
    ...
    if(v1_endpoints_list->metadata) {
    mazu_cJSON *metadata_local_JSON = v1_list_meta_convertToJSON(v1_endpoints_list->metadata);
...
v1_endpoints_list_t *v1_endpoints_list_parseFromJSON(mazu_cJSON *v1_endpoints_listJSON){
    ...
    // v1_endpoints_list->api_version
    mazu_cJSON *api_version = mazu_cJSON_GetObjectItemCaseSensitive(v1_endpoints_listJSON, "apiVersion");
    if (api_version) { 
    ...
    // v1_endpoints_list->items
    mazu_cJSON *items = mazu_cJSON_GetObjectItemCaseSensitive(v1_endpoints_listJSON, "items");
    if (!items) {
        goto end;
    }
    ...
    // v1_endpoints_list->kind
    mazu_cJSON *kind = mazu_cJSON_GetObjectItemCaseSensitive(v1_endpoints_listJSON, "kind");
    if (kind) { 
    ...
    // v1_endpoints_list->metadata
    mazu_cJSON *metadata = mazu_cJSON_GetObjectItemCaseSensitive(v1_endpoints_listJSON, "metadata");
    if (metadata) { 
