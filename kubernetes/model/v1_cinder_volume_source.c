...
mazu_cJSON *v1_cinder_volume_source_convertToJSON(v1_cinder_volume_source_t *v1_cinder_volume_source) {
    mazu_cJSON *item = mazu_cJSON_CreateObject();

    // v1_cinder_volume_source->fs_type
    if(v1_cinder_volume_source->fs_type) {
    if(mazu_cJSON_AddStringToObject(item, "fsType", v1_cinder_volume_source->fs_type) == NULL) {
    goto fail; //String
    }
    }
    ...
    if(mazu_cJSON_AddBoolToObject(item, "readOnly", v1_cinder_volume_source->read_only) == NULL) {
    goto fail; //Bool
    }
    }
    ...
    if(v1_cinder_volume_source->secret_ref) {
    mazu_cJSON *secret_ref_local_JSON = v1_local_object_reference_convertToJSON(v1_cinder_volume_source->secret_ref);
    ...
    if(mazu_cJSON_AddStringToObject(item, "volumeID", v1_cinder_volume_source->volume_id) == NULL) {
    goto fail; //String
    }
...
v1_cinder_volume_source_t *v1_cinder_volume_source_parseFromJSON(mazu_cJSON *v1_cinder_volume_sourceJSON){
    ...
    // v1_cinder_volume_source->fs_type
    mazu_cJSON *fs_type = mazu_cJSON_GetObjectItemCaseSensitive(v1_cinder_volume_sourceJSON, "fsType");
    if (fs_type) { 
    ...
    // v1_cinder_volume_source->read_only
    mazu_cJSON *read_only = mazu_cJSON_GetObjectItemCaseSensitive(v1_cinder_volume_sourceJSON, "readOnly");
    if (read_only) { 
    ...
    // v1_cinder_volume_source->secret_ref
    mazu_cJSON *secret_ref = mazu_cJSON_GetObjectItemCaseSensitive(v1_cinder_volume_sourceJSON, "secretRef");
    if (secret_ref) { 
    secret_ref_local_nonprim = v1_local_object_reference_parseFromJSON(secret_ref); //nonprimitive
    }

    // v1_cinder_volume_source->volume_id
    mazu_cJSON *volume_id = mazu_cJSON_GetObjectItemCaseSensitive(v1_cinder_volume_sourceJSON, "volumeID");
    if (!volume_id) {
        goto end;
    }
