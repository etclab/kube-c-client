...
mazu_cJSON *v1_resource_quota_list_convertToJSON(v1_resource_quota_list_t *v1_resource_quota_list) {
    ...
    if(v1_resource_quota_list->api_version) {
    if(mazu_cJSON_AddStringToObject(item, "apiVersion", v1_resource_quota_list->api_version) == NULL) {
    goto fail; //String
    }
    }
    ...
    mazu_cJSON *itemLocal = v1_resource_quota_convertToJSON(itemsListEntry->data);
    if(itemLocal == NULL) {
    goto fail;
    }
    ...
    if(v1_resource_quota_list->kind) {
    if(mazu_cJSON_AddStringToObject(item, "kind", v1_resource_quota_list->kind) == NULL) {
    goto fail; //String
    }
    }
    ...
    if(v1_resource_quota_list->metadata) {
    mazu_cJSON *metadata_local_JSON = v1_list_meta_convertToJSON(v1_resource_quota_list->metadata);
...
v1_resource_quota_list_t *v1_resource_quota_list_parseFromJSON(mazu_cJSON *v1_resource_quota_listJSON){
    ...
    // define the local list for v1_resource_quota_list->items
    ...
    // v1_resource_quota_list->api_version
    mazu_cJSON *api_version = mazu_cJSON_GetObjectItemCaseSensitive(v1_resource_quota_listJSON, "apiVersion");
    if (api_version) { 
    ...
    // v1_resource_quota_list->items
    mazu_cJSON *items = mazu_cJSON_GetObjectItemCaseSensitive(v1_resource_quota_listJSON, "items");
    if (!items) {
        goto end;
    }
    ...
    // v1_resource_quota_list->kind
    mazu_cJSON *kind = mazu_cJSON_GetObjectItemCaseSensitive(v1_resource_quota_listJSON, "kind");
    if (kind) { 
    ...
    // v1_resource_quota_list->metadata
    mazu_cJSON *metadata = mazu_cJSON_GetObjectItemCaseSensitive(v1_resource_quota_listJSON, "metadata");
    if (metadata) { 
