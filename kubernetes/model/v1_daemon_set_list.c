...
mazu_cJSON *v1_daemon_set_list_convertToJSON(v1_daemon_set_list_t *v1_daemon_set_list) {
    mazu_cJSON *item = mazu_cJSON_CreateObject();

    // v1_daemon_set_list->api_version
    if(v1_daemon_set_list->api_version) {
    if(mazu_cJSON_AddStringToObject(item, "apiVersion", v1_daemon_set_list->api_version) == NULL) {
    goto fail; //String
    }
    }
    ...
    mazu_cJSON *itemLocal = v1_daemon_set_convertToJSON(itemsListEntry->data);
    if(itemLocal == NULL) {
    goto fail;
    }
    ...
    if(v1_daemon_set_list->kind) {
    if(mazu_cJSON_AddStringToObject(item, "kind", v1_daemon_set_list->kind) == NULL) {
    goto fail; //String
    }
    }
    ...
    if(v1_daemon_set_list->metadata) {
    mazu_cJSON *metadata_local_JSON = v1_list_meta_convertToJSON(v1_daemon_set_list->metadata);
...
v1_daemon_set_list_t *v1_daemon_set_list_parseFromJSON(mazu_cJSON *v1_daemon_set_listJSON){
    ...
    // v1_daemon_set_list->api_version
    mazu_cJSON *api_version = mazu_cJSON_GetObjectItemCaseSensitive(v1_daemon_set_listJSON, "apiVersion");
    if (api_version) { 
    ...
    // v1_daemon_set_list->items
    mazu_cJSON *items = mazu_cJSON_GetObjectItemCaseSensitive(v1_daemon_set_listJSON, "items");
    if (!items) {
        goto end;
    }
    ...
    // v1_daemon_set_list->kind
    mazu_cJSON *kind = mazu_cJSON_GetObjectItemCaseSensitive(v1_daemon_set_listJSON, "kind");
    if (kind) { 
    ...
    // v1_daemon_set_list->metadata
    mazu_cJSON *metadata = mazu_cJSON_GetObjectItemCaseSensitive(v1_daemon_set_listJSON, "metadata");
    if (metadata) { 
