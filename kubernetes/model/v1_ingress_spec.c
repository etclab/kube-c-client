...
mazu_cJSON *v1_ingress_spec_convertToJSON(v1_ingress_spec_t *v1_ingress_spec) {
    mazu_cJSON *item = mazu_cJSON_CreateObject();

    // v1_ingress_spec->default_backend
    if(v1_ingress_spec->default_backend) {
    mazu_cJSON *default_backend_local_JSON = v1_ingress_backend_convertToJSON(v1_ingress_spec->default_backend);
    if(default_backend_local_JSON == NULL) {
    goto fail; //model
    }
    mazu_cJSON_AddItemToObject(item, "defaultBackend", default_backend_local_JSON);
    if(item->child == NULL) {
    goto fail;
    }
    }
    ...
    if(v1_ingress_spec->ingress_class_name) {
    if(mazu_cJSON_AddStringToObject(item, "ingressClassName", v1_ingress_spec->ingress_class_name) == NULL) {
    goto fail; //String
    }
    }
    ...
    mazu_cJSON *itemLocal = v1_ingress_rule_convertToJSON(rulesListEntry->data);
    if(itemLocal == NULL) {
    goto fail;
    }
    ...
    // v1_ingress_spec->tls
    if(v1_ingress_spec->tls) {
    mazu_cJSON *tls = mazu_cJSON_AddArrayToObject(item, "tls");
    if(tls == NULL) {
    goto fail; //nonprimitive container
    }
    ...
    mazu_cJSON *itemLocal = v1_ingress_tls_convertToJSON(tlsListEntry->data);
    if(itemLocal == NULL) {
    goto fail;
    }
    mazu_cJSON_AddItemToArray(tls, itemLocal);
    }
    }
    }
...
v1_ingress_spec_t *v1_ingress_spec_parseFromJSON(mazu_cJSON *v1_ingress_specJSON){
    ...
    // v1_ingress_spec->default_backend
    mazu_cJSON *default_backend = mazu_cJSON_GetObjectItemCaseSensitive(v1_ingress_specJSON, "defaultBackend");
    if (default_backend) { 
    default_backend_local_nonprim = v1_ingress_backend_parseFromJSON(default_backend); //nonprimitive
    }

    // v1_ingress_spec->ingress_class_name
    mazu_cJSON *ingress_class_name = mazu_cJSON_GetObjectItemCaseSensitive(v1_ingress_specJSON, "ingressClassName");
    if (ingress_class_name) { 
    if(!mazu_cJSON_IsString(ingress_class_name) && !mazu_cJSON_IsNull(ingress_class_name))
    {
    goto end; //String
    }
    }
    ...
    // v1_ingress_spec->rules
    mazu_cJSON *rules = mazu_cJSON_GetObjectItemCaseSensitive(v1_ingress_specJSON, "rules");
    if (rules) { 
    ...
    // v1_ingress_spec->tls
    mazu_cJSON *tls = mazu_cJSON_GetObjectItemCaseSensitive(v1_ingress_specJSON, "tls");
    if (tls) { 
    mazu_cJSON *tls_local_nonprimitive = NULL;
    if(!mazu_cJSON_IsArray(tls)){
        goto end; //nonprimitive container
    }

    tlsList = list_createList();

    mazu_cJSON_ArrayForEach(tls_local_nonprimitive,tls )
    {
        if(!mazu_cJSON_IsObject(tls_local_nonprimitive)){
            goto end;
        }
    ...
    v1_ingress_spec_local_var = v1_ingress_spec_create (
        ...
        ingress_class_name && !mazu_cJSON_IsNull(ingress_class_name) ? strdup(ingress_class_name->valuestring) : NULL,
