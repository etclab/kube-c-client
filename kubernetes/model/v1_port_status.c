...
mazu_cJSON *v1_port_status_convertToJSON(v1_port_status_t *v1_port_status) {
    mazu_cJSON *item = mazu_cJSON_CreateObject();

    // v1_port_status->error
    if(v1_port_status->error) {
    if(mazu_cJSON_AddStringToObject(item, "error", v1_port_status->error) == NULL) {
    goto fail; //String
    }
    }
    ...
    if(mazu_cJSON_AddNumberToObject(item, "port", v1_port_status->port) == NULL) {
    goto fail; //Numeric
    }
    ...
    if(mazu_cJSON_AddStringToObject(item, "protocol", v1_port_status->protocol) == NULL) {
    goto fail; //String
    }
...
v1_port_status_t *v1_port_status_parseFromJSON(mazu_cJSON *v1_port_statusJSON){

    v1_port_status_t *v1_port_status_local_var = NULL;

    // v1_port_status->error
    mazu_cJSON *error = mazu_cJSON_GetObjectItemCaseSensitive(v1_port_statusJSON, "error");
    if (error) { 
    ...
    // v1_port_status->port
    mazu_cJSON *port = mazu_cJSON_GetObjectItemCaseSensitive(v1_port_statusJSON, "port");
    if (!port) {
        goto end;
    }
    ...
    // v1_port_status->protocol
    mazu_cJSON *protocol = mazu_cJSON_GetObjectItemCaseSensitive(v1_port_statusJSON, "protocol");
    if (!protocol) {
        goto end;
    }
