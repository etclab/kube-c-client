...
mazu_cJSON *v1_volume_attachment_convertToJSON(v1_volume_attachment_t *v1_volume_attachment) {
    mazu_cJSON *item = mazu_cJSON_CreateObject();

    // v1_volume_attachment->api_version
    if(v1_volume_attachment->api_version) {
    if(mazu_cJSON_AddStringToObject(item, "apiVersion", v1_volume_attachment->api_version) == NULL) {
    goto fail; //String
    }
    }
    ...
    if(v1_volume_attachment->kind) {
    if(mazu_cJSON_AddStringToObject(item, "kind", v1_volume_attachment->kind) == NULL) {
    goto fail; //String
    }
    }
    ...
    if(v1_volume_attachment->metadata) {
    mazu_cJSON *metadata_local_JSON = v1_object_meta_convertToJSON(v1_volume_attachment->metadata);
    ...
    mazu_cJSON *spec_local_JSON = v1_volume_attachment_spec_convertToJSON(v1_volume_attachment->spec);
    ...
    if(v1_volume_attachment->status) {
    mazu_cJSON *status_local_JSON = v1_volume_attachment_status_convertToJSON(v1_volume_attachment->status);
...
v1_volume_attachment_t *v1_volume_attachment_parseFromJSON(mazu_cJSON *v1_volume_attachmentJSON){
    ...
    // v1_volume_attachment->api_version
    mazu_cJSON *api_version = mazu_cJSON_GetObjectItemCaseSensitive(v1_volume_attachmentJSON, "apiVersion");
    if (api_version) { 
    ...
    // v1_volume_attachment->kind
    mazu_cJSON *kind = mazu_cJSON_GetObjectItemCaseSensitive(v1_volume_attachmentJSON, "kind");
    if (kind) { 
    ...
    // v1_volume_attachment->metadata
    mazu_cJSON *metadata = mazu_cJSON_GetObjectItemCaseSensitive(v1_volume_attachmentJSON, "metadata");
    if (metadata) { 
    metadata_local_nonprim = v1_object_meta_parseFromJSON(metadata); //nonprimitive
    }

    // v1_volume_attachment->spec
    mazu_cJSON *spec = mazu_cJSON_GetObjectItemCaseSensitive(v1_volume_attachmentJSON, "spec");
    if (!spec) {
        goto end;
    }
    ...
    // v1_volume_attachment->status
    mazu_cJSON *status = mazu_cJSON_GetObjectItemCaseSensitive(v1_volume_attachmentJSON, "status");
    if (status) { 
