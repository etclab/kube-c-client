...
v1_lease_list_t *v1_lease_list_create(
    ...
    v1_lease_list_local_var->api_version = api_version;
...
void v1_lease_list_free(v1_lease_list_t *v1_lease_list) {
    if(NULL == v1_lease_list){
        return ;
    }
    listEntry_t *listEntry;
    if (v1_lease_list->api_version) {
        free(v1_lease_list->api_version);
        v1_lease_list->api_version = NULL;
    }
    if (v1_lease_list->items) {
        list_ForEach(listEntry, v1_lease_list->items) {
            v1_lease_free(listEntry->data);
        }
        list_freeList(v1_lease_list->items);
        v1_lease_list->items = NULL;
    }
    if (v1_lease_list->kind) {
        free(v1_lease_list->kind);
        v1_lease_list->kind = NULL;
    }
    if (v1_lease_list->metadata) {
        v1_list_meta_free(v1_lease_list->metadata);
        v1_lease_list->metadata = NULL;
    }
    free(v1_lease_list);
}
...
mazu_cJSON *v1_lease_list_convertToJSON(v1_lease_list_t *v1_lease_list) {
    ...
    if(v1_lease_list->api_version) {
    if(mazu_cJSON_AddStringToObject(item, "apiVersion", v1_lease_list->api_version) == NULL) {
    goto fail; //String
    }
    }
    ...
    mazu_cJSON *itemLocal = v1_lease_convertToJSON(itemsListEntry->data);
    if(itemLocal == NULL) {
    goto fail;
    }
    ...
    // v1_lease_list->kind
    if(v1_lease_list->kind) {
    if(mazu_cJSON_AddStringToObject(item, "kind", v1_lease_list->kind) == NULL) {
    goto fail; //String
    }
    }
    ...
    if(v1_lease_list->metadata) {
    mazu_cJSON *metadata_local_JSON = v1_list_meta_convertToJSON(v1_lease_list->metadata);
...
v1_lease_list_t *v1_lease_list_parseFromJSON(mazu_cJSON *v1_lease_listJSON){
    ...
    // v1_lease_list->api_version
    mazu_cJSON *api_version = mazu_cJSON_GetObjectItemCaseSensitive(v1_lease_listJSON, "apiVersion");
    if (api_version) { 
    ...
    // v1_lease_list->items
    mazu_cJSON *items = mazu_cJSON_GetObjectItemCaseSensitive(v1_lease_listJSON, "items");
    if (!items) {
        goto end;
    }
    ...
    // v1_lease_list->kind
    mazu_cJSON *kind = mazu_cJSON_GetObjectItemCaseSensitive(v1_lease_listJSON, "kind");
    if (kind) { 
    ...
    // v1_lease_list->metadata
    mazu_cJSON *metadata = mazu_cJSON_GetObjectItemCaseSensitive(v1_lease_listJSON, "metadata");
    if (metadata) { 
