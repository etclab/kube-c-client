#include <stdlib.h>
#include <string.h>
#include <stdio.h>
#include "v2_object_metric_status.h"



v2_object_metric_status_t *v2_object_metric_status_create(
    v2_metric_value_status_t *current,
    v2_cross_version_object_reference_t *described_object,
    v2_metric_identifier_t *metric
    ) {
    v2_object_metric_status_t *v2_object_metric_status_local_var = malloc(sizeof(v2_object_metric_status_t));
    if (!v2_object_metric_status_local_var) {
        return NULL;
    }
    v2_object_metric_status_local_var->current = current;
    v2_object_metric_status_local_var->described_object = described_object;
    v2_object_metric_status_local_var->metric = metric;

    return v2_object_metric_status_local_var;
}


void v2_object_metric_status_free(v2_object_metric_status_t *v2_object_metric_status) {
    if(NULL == v2_object_metric_status){
        return ;
    }
    listEntry_t *listEntry;
    if (v2_object_metric_status->current) {
        v2_metric_value_status_free(v2_object_metric_status->current);
        v2_object_metric_status->current = NULL;
    }
    if (v2_object_metric_status->described_object) {
        v2_cross_version_object_reference_free(v2_object_metric_status->described_object);
        v2_object_metric_status->described_object = NULL;
    }
    if (v2_object_metric_status->metric) {
        v2_metric_identifier_free(v2_object_metric_status->metric);
        v2_object_metric_status->metric = NULL;
    }
    free(v2_object_metric_status);
}

mazu_cJSON *v2_object_metric_status_convertToJSON(v2_object_metric_status_t *v2_object_metric_status) {
    mazu_cJSON *item = mazu_cJSON_CreateObject();

    // v2_object_metric_status->current
    if (!v2_object_metric_status->current) {
        goto fail;
    }
    mazu_cJSON *current_local_JSON = v2_metric_value_status_convertToJSON(v2_object_metric_status->current);
    if(current_local_JSON == NULL) {
    goto fail; //model
    }
    mazu_cJSON_AddItemToObject(item, "current", current_local_JSON);
    if(item->child == NULL) {
    goto fail;
    }


    // v2_object_metric_status->described_object
    if (!v2_object_metric_status->described_object) {
        goto fail;
    }
    mazu_cJSON *described_object_local_JSON = v2_cross_version_object_reference_convertToJSON(v2_object_metric_status->described_object);
    if(described_object_local_JSON == NULL) {
    goto fail; //model
    }
    mazu_cJSON_AddItemToObject(item, "describedObject", described_object_local_JSON);
    if(item->child == NULL) {
    goto fail;
    }


    // v2_object_metric_status->metric
    if (!v2_object_metric_status->metric) {
        goto fail;
    }
    mazu_cJSON *metric_local_JSON = v2_metric_identifier_convertToJSON(v2_object_metric_status->metric);
    if(metric_local_JSON == NULL) {
    goto fail; //model
    }
    mazu_cJSON_AddItemToObject(item, "metric", metric_local_JSON);
    if(item->child == NULL) {
    goto fail;
    }

    return item;
fail:
    if (item) {
        mazu_cJSON_Delete(item);
    }
    return NULL;
}

v2_object_metric_status_t *v2_object_metric_status_parseFromJSON(mazu_cJSON *v2_object_metric_statusJSON){

    v2_object_metric_status_t *v2_object_metric_status_local_var = NULL;

    // define the local variable for v2_object_metric_status->current
    v2_metric_value_status_t *current_local_nonprim = NULL;

    // define the local variable for v2_object_metric_status->described_object
    v2_cross_version_object_reference_t *described_object_local_nonprim = NULL;

    // define the local variable for v2_object_metric_status->metric
    v2_metric_identifier_t *metric_local_nonprim = NULL;

    // v2_object_metric_status->current
    mazu_cJSON *current = mazu_cJSON_GetObjectItemCaseSensitive(v2_object_metric_statusJSON, "current");
    if (!current) {
        goto end;
    }

    
    current_local_nonprim = v2_metric_value_status_parseFromJSON(current); //nonprimitive

    // v2_object_metric_status->described_object
    mazu_cJSON *described_object = mazu_cJSON_GetObjectItemCaseSensitive(v2_object_metric_statusJSON, "describedObject");
    if (!described_object) {
        goto end;
    }

    
    described_object_local_nonprim = v2_cross_version_object_reference_parseFromJSON(described_object); //nonprimitive

    // v2_object_metric_status->metric
    mazu_cJSON *metric = mazu_cJSON_GetObjectItemCaseSensitive(v2_object_metric_statusJSON, "metric");
    if (!metric) {
        goto end;
    }

    
    metric_local_nonprim = v2_metric_identifier_parseFromJSON(metric); //nonprimitive


    v2_object_metric_status_local_var = v2_object_metric_status_create (
        current_local_nonprim,
        described_object_local_nonprim,
        metric_local_nonprim
        );

    return v2_object_metric_status_local_var;
end:
    if (current_local_nonprim) {
        v2_metric_value_status_free(current_local_nonprim);
        current_local_nonprim = NULL;
    }
    if (described_object_local_nonprim) {
        v2_cross_version_object_reference_free(described_object_local_nonprim);
        described_object_local_nonprim = NULL;
    }
    if (metric_local_nonprim) {
        v2_metric_identifier_free(metric_local_nonprim);
        metric_local_nonprim = NULL;
    }
    return NULL;

}
