...
v1_type_checking_t *v1_type_checking_create(
    list_t *expression_warnings
    ) {
    v1_type_checking_t *v1_type_checking_local_var = malloc(sizeof(v1_type_checking_t));
    if (!v1_type_checking_local_var) {
        return NULL;
    }
    v1_type_checking_local_var->expression_warnings = expression_warnings;

    return v1_type_checking_local_var;
}


void v1_type_checking_free(v1_type_checking_t *v1_type_checking) {
    if(NULL == v1_type_checking){
        return ;
    }
    listEntry_t *listEntry;
    if (v1_type_checking->expression_warnings) {
        list_ForEach(listEntry, v1_type_checking->expression_warnings) {
            v1_expression_warning_free(listEntry->data);
        }
        list_freeList(v1_type_checking->expression_warnings);
        v1_type_checking->expression_warnings = NULL;
    }
...
v1_type_checking_t *v1_type_checking_parseFromJSON(mazu_cJSON *v1_type_checkingJSON){
    ...
    // define the local list for v1_type_checking->expression_warnings
    ...
    v1_type_checking_local_var = v1_type_checking_create (
